...
STATISTIC(FusionNumberNotMatch, "Fusion number does not match");

unsigned int FuseNumber = 0;
...
static cl::opt<unsigned> FusionPeelMaxCount(
    "loop-fuse-peel-max-count", cl::init(0), cl::Hidden,
    cl::desc("Max number of iterations to be peeled from a loop, such that "
             "fusion can take place"));

static cl::opt<unsigned> LoopFuseNumber(
    "loop-fuse-number", cl::init(0), cl::Hidden,
    cl::desc("Number of candidates to be fused of the possible ones to fuse."));

#ifndef NDEBUG
static cl::opt<bool>
    VerboseFusionDebugging("loop-fuse-verbose-debug",
    ...
    for (auto &CandidateSet : FusionCandidates) {
      ...
      for (auto FC0 = CandidateSet.begin(); FC0 != CandidateSet.end(); ++FC0) {
        ...
        for (++FC1; FC1 != CandidateSet.end(); ++FC1) {
          ...
          if (LoopFuseNumber){
            FuseNumber++;
            if (LoopFuseNumber != FuseNumber) {
              LLVM_DEBUG(dbgs()
                         << "Skipped. Fusion number does not match\n");
              reportLoopFusion<OptimizationRemarkMissed>(*FC0, *FC1,
                                                         FusionNumberNotMatch);
              continue;
            }
          }
...
static RegisterPass<LoopFuseLegacy>
    X("legacy-loop-fuse", "Loop Fuse",
      false, // This pass doesn't modify the CFG => true
      false // This pass is not a pure analysis pass => false
    );

INITIALIZE_PASS_BEGIN(LoopFuseLegacy, "loop-fuse", "Loop Fuse", false,
                      false)
...
INITIALIZE_PASS_END(LoopFuseLegacy, "loop-fuse", "Loop Fuse", false, false)
