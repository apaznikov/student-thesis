...
int main()
{
	...
	mars.inp_key(T);
	do {
		std::cout << std::endl << "Выберите действие:\n0-Выход\n1-Зашифровать\n2-Дешифровать" << std::endl;
		std::cin >> c;
		if (c == 1) {

			std::cout << "Шифровка сообщения" << std::endl << "Введите текст для шифровки" << std::endl;
			getline(std::cin, s);
			getline(std::cin, s);
			V = StringToInt(s);
			mars.code(V);
			Bl = mars.blocks;
			V = mars.GetBlocks();
			std::cout << "Количество блоков:" << V.size() / 4 << std::endl << "Зашифрованное сообщение:";
