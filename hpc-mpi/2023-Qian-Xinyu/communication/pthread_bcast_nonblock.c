...
void *sender_thread_func(void *arg) {
    ...
    MPI_Request *request = tdata->request;

    MPI_Isend(value, 3000 * 3000, MPI_INT, tid, 0, MPI_COMM_WORLD, &request[tid - 1]);
    pthread_exit(NULL);
}
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        MPI_Request request;
        MPI_Irecv(buf, 3000 * 3000, MPI_INT, 0, 0, MPI_COMM_WORLD, &request);
